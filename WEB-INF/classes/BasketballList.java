...
public class BasketballList extends HttpServlet {
 ...
 String command =
  "curl,-X,GET,--header,Authorization: Bearer UGE6FGLedRgWsGhyRSlj21goTGxD,--header,Accept: application/json,https://api.stubhub.com/sellers/search/events/v3?q=Basketball&rows=350";
...
InputStream inputStream = process.getInputStream();
        ...
        String result = data.convertInputStreamToString(inputStream);
            ...
            PrintWriter out = new PrintWriter(new FileWriter("C:\\Illinois tech\\EWA\\apache-tomcat-7.0.34\\webapps\\Sportech\\WEB-INF\\classes\\Basketball.json"));
            out.write(result);
}
      ...
      data.doPost(Basketball,basket);
		...
		pw.print("<a style='font-size: 24px;'>"+name+" Basketball</a>");
		pw.print("</h2><div class='entry'><table id='bestseller'>");
		int i = 1; int size= hm.size();
		for(Map.Entry<String, Basketball> entry : hm.entrySet())
		{
			Basketball basketball = entry.getValue();
  ...
  pw.print("<a class='card'>");
    ...
    pw.print("<div class='card-content'>");
      pw.print("<div class='card-body-title'>");
        pw.print("<h2>"+basketball.getName()+"</h2>");
      pw.print("</div>");
      pw.print("<div class='card-body'>");
        pw.print("<p>"+basketball.getEventDate());
        pw.print("<br/>");
			pw.print(basketball.getVenue());
			pw.print("<br/>");
			pw.print(basketball.getCity());
			pw.print("<br/>");
			pw.print(basketball.getState());
