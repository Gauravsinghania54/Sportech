...
@WebServlet("/HockeyList")

public class HockeyList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		String name = null;
		String command =
  "curl,-X,GET,--header,Authorization: Bearer UGE6FGLedRgWsGhyRSlj21goTGxD,--header,Accept: application/json,https://api.stubhub.com/sellers/search/events/v3?q=Hockey&rows=500,";
...
InputStream inputStream = process.getInputStream();
        ...
        String result = data.convertInputStreamToString(inputStream);
            ...
            PrintWriter out = new PrintWriter(new FileWriter("C:\\Illinois tech\\EWA\\apache-tomcat-7.0.34\\webapps\\Sportech\\WEB-INF\\classes\\Hockey.json"));
            out.write(result);
}
      ...
      data.doPost(Basketball,basket);
		HashMap<String,Hockey> allhockeys = new HashMap<String,Hockey> ();

		try{
		     allhockeys = MySqlDataStoreUtilities.getHockeys();
		}
		catch(Exception e)
		{
		...
		pw.print("<a style='font-size: 24px;'>"+name+" Hockey</a>");
		pw.print("</h2><div class='entry'><table id='bestseller'>");
		int i = 1; int size= hm.size();
		for(Map.Entry<String, Hockey> entry : hm.entrySet())
		{
	Hockey hockey = entry.getValue();
    ...
    pw.print("<div class='card-content'>");
      pw.print("<div class='card-body-title'>");
        pw.print("<h2>"+hockey.getName()+"</h2>");
      pw.print("</div>");
      pw.print("<div class='card-body'>");
        pw.print("<p>"+hockey.getEventDate());
        pw.print("<br/>");
			pw.print(hockey.getVenue());
			pw.print("<br/>");
			pw.print(hockey.getCity());
			pw.print("<br/>");
			pw.print(hockey.getState());
